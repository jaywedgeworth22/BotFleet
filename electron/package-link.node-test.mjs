...
describe("BotMRR package deep links", () => {
  ...
  it("rejects other commands, hosts, protocols, credentials, and unsupported file types", () => {
    assert.equal(packageUrlFromDeepLink("botfleet://settings"), null);
    assert.equal(packageUrlFromDeepLink("botfleet://install?url=https://evil.example/bot.json"), null);
    assert.equal(packageUrlFromDeepLink("botfleet://install?url=http://raw.githubusercontent.com/a/b/main/bot.json"), null);
    assert.equal(packageUrlFromDeepLink("botfleet://install?url=https://user@example.com/bot.json"), null);
    assert.equal(packageUrlFromDeepLink("botfleet://install?url=https://github.com/acme/bot/run.sh"), null);
  });
});
