...
//    EmbeddedCuaDriverHost so TCC grants attribute to BotFleet and the
//    driver inherits them. One prompt, named BotFleet, out of the box.
...
async function loadEmbeddedSdk() {
  ...
  process.env.BOTFLEET_CUA_SDK_LIBRARY = path.join(
...
export async function startCua() {
  ...
  const wantEmbedded =
    app.isPackaged || process.env.BOTFLEET_CUA_EMBEDDED === "1";
  let nextConnection;
