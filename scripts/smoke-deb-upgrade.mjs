...
try {
  const status = execFileSync("dpkg-query", ["-W", "-f=${db:Status-Abbrev}", "botfleet"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing BotFleet installation");
} catch (error) {
...
const legacyApp = path.join(legacyRoot, "opt", "BotFleet");
const legacyResources = path.join(legacyApp, "resources");
const legacyDeb = path.join(temporary, "botfleet_0.1.7_amd64.deb");
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: BotFleet CI <ci@botfleet.invalid>",
      "Description: Legacy BotFleet directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  ...
  for (const directory of ["/opt/BotFleet", "/opt/BotFleet/resources"]) {
  ...
  for (const directory of [
    "/opt/BotFleet",
    "/opt/BotFleet/resources",
    "/opt/BotFleet/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/BotFleet/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/BotFleet/chrome-sandbox";
  ...
  const installedVersion = execFileSync("dpkg-query", ["-W", "-f=${Version}", "botfleet"], {
    encoding: "utf8",
  }).trim();
  console.log(
