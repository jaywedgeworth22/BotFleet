        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          ...
          <details className="group rounded-xl border border-hairline/45 bg-inset/45 px-4 py-3" open={Boolean(webhook)}>
            ...
            <div className="mt-4 space-y-4">
              ...
              <label className="block"><span className="mb-1.5 block text-[11.5px] font-medium text-ink-secondary">Handling Rule <span className="font-normal">· optional</span></span><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={3} placeholder="For every event, summarize what happened and suggest the next step…" className="w-full resize-y rounded-xl border border-hairline/60 bg-panel px-3.5 py-3 text-[13px] leading-relaxed text-ink outline-none placeholder:text-ink-secondary/60 focus:border-accent/70" /><span className="mt-1.5 block text-[10.5px] leading-relaxed text-ink-secondary">Use this only when every event needs the same handling rule.{'\u00A0 '}Otherwise the request’s task is used.</span></label>
              <div><div className="mb-2 text-[11.5px] font-medium text-ink-secondary">Run On</div><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setRunOn("bot")} className={cn("rounded-xl border p-3 text-left", runOn === "bot" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-panel hover:bg-raised/60")}><div className="flex items-center gap-2 text-[12.5px] font-medium text-ink"><Laptop size={14} />This Computer</div></button><button type="button" disabled={!cloudReady && runOn !== "cloud"} onClick={() => setRunOn("cloud")} className={cn("rounded-xl border p-3 text-left disabled:cursor-not-allowed disabled:opacity-45", runOn === "cloud" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-panel hover:bg-raised/60")}><div className="flex items-center gap-2 text-[12.5px] font-medium text-ink"><Cloud size={14} />ASCII.dev Box</div></button></div></div>
            ...
            <section className="min-w-0">
              <div className="flex flex-wrap items-start justify-between gap-3 px-5 pt-5 md:px-7 md:pt-6">
                <div className="flex min-w-0 items-center gap-3">
                  {selectedBot ? <BotAvatar bot={selectedBot} state={selected.enabled ? stateForBot(selectedBot) : "sleeping"} size={44} animated={false} label={selectedBot.name} /> : <div className="flex size-11 items-center justify-center rounded-xl bg-raised text-ink-secondary"><Webhook size={18} /></div>}
                  <div className="min-w-0">
                    <h3 className="truncate text-[17px] font-semibold text-ink" title={selected.name}>{selected.name}</h3>
                    <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[10.5px] text-ink-secondary"><span>{selectedBot?.name ?? "Deleted bot"}</span><span>·</span><span>{selected.runOn === "cloud" ? "ASCII.dev Box" : "This computer"}</span></div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
