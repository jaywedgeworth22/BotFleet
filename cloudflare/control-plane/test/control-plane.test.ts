...
const BASE_URL = "https://auth.botfleet.test";
...
describe("control-plane migrations and health", () => {
  ...
  it("serves a no-store health response without CORS wildcards", async () => {
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ ok: true, service: "botfleet-control-plane" });
...
describe("HTTP boundary hardening", () => {
  ...
  it("defaults CORS to deny and never emits a wildcard", async () => {
    ...
    const allowed = await call("/v1/me", { origin: "https://app.botfleet.test" });
    expect(allowed.status).toBe(401);
    expect(allowed.headers.get("access-control-allow-origin")).toBe("https://app.botfleet.test");
    expect(allowed.headers.get("cache-control")).toBe("no-store");

    const deniedPreflight = await call("/v1/installations", {
      method: "OPTIONS",
      origin: "https://app.botfleet.test",
      headers: {
    ...
    expect(deniedPreflight.headers.get("access-control-allow-origin")).toBe("https://app.botfleet.test");
  });
});
