...
```sh
pnpm --filter @botfleet/control-plane exec wrangler d1 migrations apply DB --local --config wrangler.jsonc
pnpm --filter @botfleet/control-plane exec wrangler dev --config wrangler.jsonc
```
...
8. Deploy the Worker and verify that `GET <BETTER_AUTH_URL>/healthz` returns
   exactly `{ "ok": true, "service": "botfleet-control-plane" }` over
