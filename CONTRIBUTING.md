...
- stable `BotFleet-amd64.deb` and `BotFleet.AppImage` copies used by the latest-download links;
...
[`botfleet-releases`](https://github.com/jaywedgeworth22/botfleet-releases) repository.  Then verify the checksum
...
Data lives in `~/.botfleet/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.botfleet`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.botfleet/config.json` via `PUT /api/config` and the API
