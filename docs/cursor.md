...
Cursor is an optional BotFleet engine. BotFleet runs the official
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; BotFleet already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, BotFleet pins the same id over
