# Computer use & browser use in BotFleet

Decision doc, 2026-08-12. How bots in BotFleet get local computer use and
...
supply-chain contract tracked in [#113](https://github.com/milind-soni/BotFleet/issues/113): pinned archive
...
receives the validated MCP proxy contract. Xorg is tracked in [#79](https://github.com/milind-soni/BotFleet/issues/79);
GNOME/Wayland additionally requires WinRects v8 plus the exact Cua health-report contract tracked in
[#109](https://github.com/milind-soni/BotFleet/issues/109).
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is BotFleet's, users see ONE
   prompt named BotFleet, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `BotFleet.app/Contents/Resources/cua-driver`,
