...
BotFleet has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
- The native Electron window and embedded BotFleet server on GNOME Xorg and GNOME Wayland.
...
progress in [issue #29](https://github.com/milind-soni/BotFleet/issues/29) and the safety hold in
[issue #345](https://github.com/milind-soni/BotFleet/issues/345). Bundled
CUA supply-chain work is tracked in [issue #113](https://github.com/milind-soni/BotFleet/issues/113). Xorg is tracked in
[issue #79](https://github.com/milind-soni/BotFleet/issues/79), and guarded GNOME/Wayland support in
[issue #109](https://github.com/milind-soni/BotFleet/issues/109).
...
- `release/BotFleet-<version>-amd64.deb`
- `release/BotFleet-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./BotFleet-amd64.deb
```

Then open **BotFleet** from the GNOME application launcher. To remove it:

```sh
sudo apt remove botfleet
```

The portable AppImage does not install system files:

```sh
chmod +x release/BotFleet-*-x86_64.AppImage
./release/BotFleet-*-x86_64.AppImage
```

For a downloaded release AppImage, use `BotFleet.AppImage` in place of the versioned path above.

Application data remains local in `~/.botfleet`. Electron browser data and window state use the normal XDG
configuration directory (`~/.config/botfleet` unless the environment overrides it).
...
./release/linux-unpacked/botfleet
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. BotFleet
...
```sh
OMB_EXTRA_PATH=/your/custom/bin ./release/BotFleet-*-x86_64.AppImage
```

Restart BotFleet after installing or signing in to a CLI.
...
24.04 GNOME Wayland session. BotFleet detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. BotFleet does not capture screen audio, remember the selected monitor after restart, or
...
The Xorg runtime uses private sockets, standard permission mode, per-action BotFleet approvals,
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart BotFleet. If it lives outside the
...
```sh
chmod +x BotFleet-*-x86_64.AppImage
file BotFleet-*-x86_64.AppImage
```
