...
The iOS app is a thin, native client for the BotFleet instance running on
...
an BotFleet account of its own.
...
The sidecar keeps its device registry in `~/.botfleet/devices.json`. That is
...
Tailscale** in the desktop setup alternatives. BotFleet then places the
...
The BotFleet control plane stores account and installation metadata plus
...
The Mac must remain awake with BotFleet running for chats, approvals, and
...
An BotFleet account is not required for nearby, manual, or Tailscale
