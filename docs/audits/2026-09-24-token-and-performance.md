...
|---|---|---|
| Chat-completions history is replayed without a byte cap | `server/index.ts` selects up to 40 text messages for MiniMax, Grok, and OpenAI-compatible drivers, but a single prior message may exceed a provider context window.  Those drivers resend the history on each API request and tool round. | This branch caps prior replay at 128 KiB, keeps recent complete messages, clips one oversized newest message, and marks omitted history.  The current user prompt and native CLI session path are untouched.  Board `93e15d9a`, issue #540. |
