...
| C6 | README paths fixed, tag 0.1.37, circular fork copy | Partial | Fork copy fixed in #95.  **Dispute** the surrounding premise: every README download link points at `jaywedgeworth22/botfleet-releases`, a repository that does not exist (GitHub 404), so the README links are dead regardless of the tag. |
| C7 | Site Download CTA hits source-repo Releases, artifacts do not live there | **Dispute** | The only release anywhere is `jaywedgeworth22/BotFleet` v0.1.38 (two DMGs).  `milind-soni/botfleet-releases` and `jaywedgeworth22/botfleet-releases` both 404.  The site CTA (`…/BotFleet/releases/latest`) was therefore the one working download link, and PR #95's retarget to `botfleet-releases` made it worse.  Correction: point README and site at `…/BotFleet/releases/latest`, and retarget `electron-builder.yml` publish (still `milind-soni/botfleet-releases`) so auto-update can ever work.  Batch B3b. |
...
| Table 11: "Download for Mac" points at the wrong repo | **Dispute** | See C7. |
...
- **Release truth.**  The only GitHub release is `jaywedgeworth22/BotFleet` v0.1.38 (2026-08-31, assets: `BotFleet-0.1.38-arm64.dmg`, `BotFleet-0.1.38-x64.dmg`; no zip, no `latest-mac.yml`, no Windows or Linux artifacts).  `jaywedgeworth22/botfleet-releases` and `milind-soni/botfleet-releases` both return 404.  So the site CTA (`…/BotFleet/releases/latest`) is the working link and every README download link (v0.1.37 on botfleet-releases) is dead.  `electron-builder.yml` publish still targets `milind-soni/botfleet-releases`; `/Applications/BotFleet.app` has no `app-update.yml`, so in-app updates cannot succeed.
