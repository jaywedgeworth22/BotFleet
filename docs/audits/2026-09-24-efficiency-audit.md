...
| RH2 | P2 | 336 remote branches with no delete-on-merge; the five oldest "unmerged by ancestry" branches were all squash-merged on Aug 31 and Sep 1.  GitHub reports 434 merged, 24 closed, 8 open PRs, so several hundred branches are dead weight. | `git branch -r | wc -l`; `gh pr list --head` spot checks | New.  Fix: enable "automatically delete head branches" on the repo and prune once |
...
15. **Trim CI and the working set (OP8, RH1, RH2, RH3, RH4).**  Per-area path filters, prune merged worktrees (about 24 GB), enable delete-on-merge, clean the integration tree, drop `.tmp-pr-body.md`.
