...
BotFleet connects only through a named alias in your `~/.ssh/config` — you type the alias into
...
**Host key first, by hand.** Connect once manually before pointing BotFleet at the alias:
...
- **No public ports.** The managed container is created with no published ports, and BotFleet refuses to
...
- **Firewall the VPS to SSH only**, ideally from your IP. Nothing BotFleet does needs any other inbound
  port open, so anything else open is pure attack surface.
- **Nothing sensitive is stored.** The only thing BotFleet persists is the alias name itself
  (`~/.botfleet/config.json`); keys, passphrases, and agent state stay with SSH. The alias is also kept
