# BotFleet harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what BotFleet lacked by
...
**The reframe that reorders everything.** BotFleet is itself a harness. The
...
**On-disk forward compatibility.** `~/.botfleet` is shared across app versions.
