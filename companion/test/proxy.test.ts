...
describe("live companion endpoint refresh", () => {
  it("requires a valid paired bearer and reflects hosted add/remove without restart", async () => {
    ...
    try {
      ...
      endpoints = [
        { kind: "hosted", priority: 0, url: "https://c-opaque.botfleet.test" },
...
describe("pairing, end to end", () => {
  it("turns a QR credential into a working device token", async () => {
    ...
    const control = createControlServer({
      devices: registry,
      companionPort: port,
      discovery: () => ({ advertising: false, name: "BotFleet" }),
    ...
    try {
      ...
      expect(body.hosts).toEqual(["macbook.tail1234.ts.net", "192.168.1.42", "botfleet-abcd1234.local"]);
