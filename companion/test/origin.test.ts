  ...
  it("accepts only the one-generation UDS or named-pipe shape", () => {
    ...
    expect(
      companionOriginSocket(
        "\\\\.\\pipe\\botfleet-companion-origin-42-12345678-1234-1234-1234-123456789abc",
        "win32",
      ),
    ).toContain("botfleet-companion-origin-42");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const server = createServer((_incoming, response) => {
        ...
        response.end(JSON.stringify({ app: "botfleet" }));
      });
      ...
      expect(JSON.parse(body)).toEqual({ app: "botfleet" });
