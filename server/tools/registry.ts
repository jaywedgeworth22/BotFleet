...
const ROUTINE_SCHEDULE_SCHEMA = {
  ...
  properties: {
    ...
    timeZone: {
      type: "string",
      description:
        "Optional IANA timezone for a weekly or daily schedule, for example America/Chicago.  On create, omit it to use the computer timezone returned by list_routines.  On update, omit it to preserve the routine's existing timezone; send a different IANA timezone to change it.",
    },
    weekdays: {
      type: "array",
