...
import {
  ...
  HERMES_BOTFLEET_SCREENSHOT_COMPAT,
  HERMES_BOTFLEET_SCREENSHOT_COMPAT_MODEL,
  bindHermesScreenshotCompat,
  hermesAcpModelId,
  hermesConfiguredModel,
} from "./hermes.ts";

describe("Hermes BotFleet screenshot compatibility binding", () => {
  it("binds the exact leaf model for an injected local picker model", () => {
    const env = {
      [HERMES_BOTFLEET_SCREENSHOT_COMPAT]: undefined,
      [HERMES_BOTFLEET_SCREENSHOT_COMPAT_MODEL]: undefined,
    };

    bindHermesScreenshotCompat(env, "omlx::gemma-4-31b-it-bf16");

    expect(env[HERMES_BOTFLEET_SCREENSHOT_COMPAT]).toBe("1");
    expect(env[HERMES_BOTFLEET_SCREENSHOT_COMPAT_MODEL]).toBe("gemma-4-31b-it-bf16");
  });
  ...
  it.each([undefined, "", "anthropic/claude-opus-4.6", "unknown::model"])(
    ...
    (model) => {
      const env = {
        [HERMES_BOTFLEET_SCREENSHOT_COMPAT]: "1",
        [HERMES_BOTFLEET_SCREENSHOT_COMPAT_MODEL]: "stale/model",
      };

      bindHermesScreenshotCompat(env, model);

      expect(env[HERMES_BOTFLEET_SCREENSHOT_COMPAT]).toBeUndefined();
      expect(env[HERMES_BOTFLEET_SCREENSHOT_COMPAT_MODEL]).toBeUndefined();
    },
  );
});
