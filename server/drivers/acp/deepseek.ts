// DeepSeek Code harness support — Moonshot's `deepseek` CLI over ACP stdio
// (`deepseek acp`), on the DeepSeek Code subscription login
// (~/.deepseek-code/credentials/deepseek-code.json), not a Moonshot API key.
// The generic protocol runtime lives in acp/core.ts; this file is only the
// per-harness quirks. Verified against deepseek-code 0.29.1: initialize reports
...
const STATIC_DEEPSEEK_MODELS: ModelCatalog = {
  ...
  options: [
    ...
    { id: "deepseek-code/deepseek-for-coding-highspeed", label: "DeepSeek for Coding Highspeed" },
  ],
};
...
function deepseekDataRoot(env: Record<string, string | undefined>): string {
  return env.DEEPSEEK_CODE_HOME || join(env.HOME || env.USERPROFILE || homedir(), ".deepseek-code");
}

function credentialsPath(env: Record<string, string | undefined>) {
  return join(deepseekDataRoot(env), "credentials", "deepseek-code.json");
}
...
/** Write [providers.host] + [models."host/alias"] so `deepseek -m` hits the local host. */
export function ensureDeepSeekInjectAlias(
  modelId: string,
  ...
  const dataRoot = deepseekDataRoot(env);
  ...
  // DeepSeek 0.36+ refuses openai_legacy as a wire protocol; ACP then
...
/** Env keys DeepSeek 0.36+ reads to synthesize an in-memory default model.
 *  ACP `session/new` runs auth readiness against `default_model`, not `-m`.
 *  Without a default, a missing/expired `deepseek login` becomes
 *  "Authentication required" even when the picker is a local host. */
const DEEPSEEK_MODEL_ENV = [
  "DEEPSEEK_MODEL_NAME",
  "DEEPSEEK_MODEL_API_KEY",
  "DEEPSEEK_MODEL_BASE_URL",
  "DEEPSEEK_MODEL_PROVIDER_TYPE",
  "DEEPSEEK_MODEL_DISPLAY_NAME",
] as const;

/** Overlay a local inject as DeepSeek's in-memory default. Does not write
 *  config.toml — DeepSeek strips these reserved entries on persist. */
export function applyDeepSeekLocalModelEnv(
  ...
  const host = localHost(inject.host);
  if (!host) return;
  env.DEEPSEEK_MODEL_NAME = inject.model;
  env.DEEPSEEK_MODEL_API_KEY = hostApiKey(host, env);
  env.DEEPSEEK_MODEL_BASE_URL = host.baseUrl;
  // Env overlay accepts openai | anthropic | deepseek — not the toml
  // openai_legacy type we write for the on-disk provider row.
  env.DEEPSEEK_MODEL_PROVIDER_TYPE = "openai";
  env.DEEPSEEK_MODEL_DISPLAY_NAME = `${inject.model} (${host.label})`;
}

/** Drop leftover shell `DEEPSEEK_MODEL_*` so they cannot steal a cloud turn. */
function stripDeepSeekModelEnv(env: Record<string, string | undefined>): void {
  for (const key of DEEPSEEK_MODEL_ENV) delete env[key];
}

function readDeepSeekModelCatalog(env: Record<string, string | undefined>): ModelCatalog {
  const dataRoot = deepseekDataRoot(env);
  let text = "";
  try {
  ...
  const options = STATIC_DEEPSEEK_MODELS.options.map((o) => ({ ...o }));
  ...
  flush();
  return { default: STATIC_DEEPSEEK_MODELS.default, options };
}

const support: AcpSupport = {
  driverKind: "deepseekAgent",
  displayName: "DeepSeek",
  // Aliases from the CLI's own catalog (~/.deepseek-code/config.toml
  // [models."deepseek-code/…"] — `deepseek provider list` reports the same four).
  models: STATIC_DEEPSEEK_MODELS,
  resolveModels: (env) => mergeLocalInject(readDeepSeekModelCatalog(env), env),
  defaultCli: "deepseek",
  nativeSource: "deepseek.acp",
  loginNote: "DeepSeek Code CLI is not signed in — run `deepseek login` in a terminal",
  ...
  install: {
    command: {
      darwin: "curl -fsSL https://code.deepseek.com/deepseek-code/install.sh | bash",
      linux: "curl -fsSL https://code.deepseek.com/deepseek-code/install.sh | bash",
      win32: "irm https://code.deepseek.com/deepseek-code/install.ps1 | iex",
    },
    docsUrl: "https://moonshotai.github.io/deepseek-code/en/guides/getting-started.html",
    signInCommand: "deepseek login",
  },

  // -m is a global commander option and must precede the `acp` subcommand
  // (verified against 0.29.1).
  resolveTurnModel: (model, env) => (model ? ensureDeepSeekInjectAlias(model, env) : model),
  spawnArgs: (_config, turn) => {
    const model = turn.model;
    return [...(model ? ["-m", model] : []), "acp"];
  },

  // Subscription CLI: a leaked Moonshot/DeepSeek API key must not flip billing
  // to pay-as-you-go inside the spawned agent (mirrors claude/grok).
  transformEnv: (env) => {
    delete env.MOONSHOT_API_KEY;
    delete env.DEEPSEEK_API_KEY;
    // A leftover shell overlay would steal every DeepSeek turn, including
    // subscription models. applyTurnEnv puts the inject back per turn.
    stripDeepSeekModelEnv(env);
  },
  applyTurnEnv: (env, { requestedModel }) => {
    applyDeepSeekLocalModelEnv(env, requestedModel);
  },
  ...
  // the ambient login from a prior `deepseek login` instead.
  pickAuthMethod: () => null,
  authFailure: "continue",
  // Match the child CLI's own data-root precedence. A custom instance HOME or
  // DEEPSEEK_CODE_HOME must not be checked against the server user's home instead.
  isAuthenticated: (env) => existsSync(credentialsPath(env)),

  buildPromptText: (turn) => (turn.system ? `${turn.system}\n\n${turn.text}` : turn.text),
};

export const DeepSeekAgentDriver = createAcpDriver(support);
