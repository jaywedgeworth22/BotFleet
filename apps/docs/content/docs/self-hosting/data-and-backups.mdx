...
description: Know what BotFleet stores locally and what to preserve.
icon: DatabaseBackup
---

BotFleet keeps its application state under `~/.botfleet`, including configuration, transcripts, message attachments, device pairing state, and the SQLite message store.

## Back up

Close BotFleet before taking a filesystem-level backup so SQLite and configuration files are captured consistently. Preserve the entire `.botfleet` directory rather than selecting individual database files.
