...
1. Quit every BotFleet window.
2. Confirm no old BotFleet process remains.
3. Start the app once and wait for the local harness.
4. If it still fails, inspect the desktop log.

On macOS, packaged logs are under `~/Library/Logs/BotFleet`. On Windows they are under the BotFleet log directory in `%APPDATA%`.
...
Developers can set `OMB_DATA_DIR` to run against a clean BotFleet data directory. This is useful for reproduction and screenshots without touching a real workspace.
