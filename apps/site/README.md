...
- `features.json` â the feature list (the only file to edit for content changes).
- `template.html` + `build.mjs` â render `index.html` from the data.
- `sync-status.mjs` â refreshes each card's PR state from GitHub and reports merged-but-unlisted PRs and promotion candidates; it never moves a card between sections on its own.
- `logo-256.png` / `icon-1024.png` / `apple-touch-icon.png` / `favicon-64.png` â the code-visor app icon (light-background rounded tile).
- `icon-transparent-1024.png` â transparent code-visor robot mark.
- `hero-bots.png` â the fleet-at-a-console hero art.  `wide-banner.png` / `wide-banner-transparent.png` â the code-visor wordmark lockup, light or transparent.
- `.well-known/apple-app-site-association` â associated-domains file for the iOS app's Universal Links (`applinks:botfleet.app`) and shared web credentials (`webcredentials:botfleet.app`); appIDs use Team `CC8UTF7ATG` / bundle `app.botfleet`.  Must stay in sync with `ios/App/BotFleet.entitlements`.
- `vercel.json` â clean URLs plus a header rule that serves the AASA file as `application/json`.

## Updating the feature list

Edit `features.json`, run `node build.mjs`, commit `index.html` too, push to `main` â Vercel deploys.  Rules:
...
- Owner copy rules apply: two spaces between sentences (a real U+00A0 plus a space in HTML strings â never the `&nbsp;` entity, which can leak as literal text), Title Case headings, light theme.
- No internal agent seat names on the public site.
- The bot roster is an example fleet, not a product claim â keep it framed that way.
